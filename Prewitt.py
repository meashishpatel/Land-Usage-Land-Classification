...
class prewittEdgeDetectionApp:
    ...
    def edge_detection(self):
        ...
        try:
            ...
            # Perform convolution using cv2.filter2D for efficient processing
            Gx = cv2.filter2D(gray_image, -1, Mx)
            Gy = cv2.filter2D(gray_image, -1, My)

            # Calculate gradient magnitude
            self.filtered_image = np.sqrt(Gx**2 + Gy**2)

            # Normalize to the range 0-255
            self.filtered_image = np.uint8(255 * self.filtered_image / np.max(self.filtered_image))
